...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            if (esp_mqtt_client_subscribe(s_client, "/topic/test", 0) < 0) {
...
void mqtt_app_publish(char* topic, char *publish_string)
{
    if (s_client) {
        ...
        ESP_LOGI(TAG, "sent publish on topic %s with value %s returned msg_id=%d", topic, publish_string, msg_id);
    }
}

void mqtt_app_start(void)
{
    // Hier MQTT-Broker Zugang eingeben
    ...
    mqtt_app_publish("/test/start", "xxx");
}
